...
class ControladorAdocao():
    ...
    def emitir_relatorio_adocoes(self):
        ...
        if len(adocoes) == 0:
            self.__tela_adocao.mostrar_mensagem("Nenhuma adoção realizada nesse período")
            return
    ...
    def avaliar_adocao(self, animal, adotante):
        if isinstance(animal, Cachorro):
            if (animal.porte == 'grande' and 
                (adotante.tipo_habitacao == 'apartamento' and 
                 adotante.tamanho_habitacao == 'pequeno'
                )
            ):
                return False
    ...
    def adotar(self):
        ...
        if adotante != None and not isinstance(adotante, Adotante):
            ...
            if cadastrar == 's':
                ...
                self.__tela_adocao.mostrar_mensagem("O processo de adoção não foi concluído")
                return
        ...
        if self.avaliar_adocao(animal, adotante):
            self.__tela_adocao.mostrar_mensagem(f"Adoção do animal {animal.nome} aprovada.")
            ...
            if assinar_termo == 's' or adocao.termo_assinado:
                ...
                self.__tela_adocao.mostrar_mensagem(f'Processo de adoção não foi concluído.')
                return
    ...
    def assinar_termo_adocao(self, adocao = None):
        ...
        self.__tela_adocao.mostrar_mensagem(f"Eu, {adocao.adotante.nome}, portador do CPF {adocao.adotante.cpf},")
        self.__tela_adocao.mostrar_mensagem(f"declaro que irei adotar o animal {adocao.animal.nome},")
        self.__tela_adocao.mostrar_mensagem(f"portador do chip {adocao.animal.chip},")
        self.__tela_adocao.mostrar_mensagem(f"e me comprometo a cuidar dele com responsabilidade, amor e carinho.")
        print()
    ...
    def abrir_tela(self):
        lista_opcoes = {1: self.adotar, 2: self.emitir_relatorio_adocoes, 3: self.assinar_termo_adocao, 0: self.retornar}
